...
<template>
  ...
  <div v-else class="max-w-4xl mx-auto rounded-xl p-1 overflow-auto"
    :class="route.path === '/compras' ? 'bg-purple-900/20' : 'h-[80vh]' && route.path === '/cuenta' ? '' : 'h-[80vh]' ">
    ...
    <div class="flex w-full p-2 border-b"
      :class="route.path === '/compras' ? ' hover:bg-purple-900/20' : 'hover:bg-gray-200'
       && route.path === '/cuenta' ? '' : 'hover:bg-gray-200' " v-for="item in cartProducts"
      :key="item.slug">
      ...
      <div class="px-4  mr-2 mt-2 w-full" :class="route.path === '/compras' ? 'flex ' : ''">
        <div :class="route.path === '/compras' ? 'w-full md:w-1/3' : ''">
          <span class="font-normal  text-center "
            :class="route.path === '/compras' ? 'text-white' : 'text-gray-800'  && route.path === '/cuenta' ? 'text-white' : 'text-gray-800' ">{{
              item.producto
            }}</span>
        </div>
          ...
          <div class="flex  gap-3  w-1/2 pb-3 h-10  mt-3 " :class="route.path === '/compras' ? 'justify-center ml-8' : ''">
            <button @click="less(item)" class=" rounded-md"
              ...
              <IconsMinus v-if="item.cantidad > 1" class="w-4 mx-1 "
                :class="route.path === '/compras' ? 'stroke-white' : 'stroke-red-600'  && route.path === '/cuenta' ? 'stroke-white' : 'stroke-red-600' " />
              <IconsDelete v-else class="w-4 mx-1 "
                :class="route.path === '/compras' ? 'stroke-white' : 'stroke-red-600'
                 && route.path === '/cuenta' ? 'stroke-white' : 'stroke-red-600 ' " />
            </button>
            <p class=" font-normal text-lg" :class="route.path === '/compras' ? 'text-gray-300' : 'text-gray-700'  && route.path === '/cuenta' ? 'text-white' : '' ">
              {{ item.cantidad }}
            </p>
            <button @click="more(item)" class="rounded-md"
              :class="route.path === '/compras' ? 'bg-transparent border hover:bg-white/20' : 'bg-gray-300'  && route.path === '/cuenta' ? 'bg-transparent border hover:bg-white/20' : 'bg-green-900/20 border border-green-800' ">
              <IconsPlus class="w-4 mx-1 " :class="route.path === '/compras' ? 'stroke-white' : 'stroke-green-800'  && route.path === '/cuenta' ? 'stroke-white' : 'stroke-green-800' " />
            </button>
          </div>
          <div class="w-full ">
    ...
    <div v-if="route.path !== '/compras'" class="flex  text-lg "
      ...
      && route.path === '/cuenta' ? 'bg-black/30' : 'bg-gray-100'">
